...
void I2C_transmitMem(uint8_t addr, uint8_t regAddr, uint8_t *p_data)
    {
    HAL_I2C_Mem_Write(&I2C1_handle, addr << 1, regAddr, sizeof(regAddr), p_data,
	    sizeof(*p_data), TIMEOUT);
 ...
 * @return uint32_t
 */
uint32_t I2C_receiveMem(uint8_t addr, uint8_t regAddr, uint8_t *p_data,
	uint8_t size)
    {
    HAL_I2C_Mem_Read(&I2C1_handle, addr << 1, regAddr, sizeof(regAddr), p_data,
	    size, TIMEOUT);
    return *p_data;
    }
...
void I2C_receive(uint8_t addr, uint8_t *p_data)
    {
    HAL_I2C_Master_Receive(&I2C1_handle, addr << 1, p_data, sizeof(p_data),
	    TIMEOUT);
    }
